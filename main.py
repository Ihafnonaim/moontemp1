...
def triplate_mean_lr(prev, pres, serv, lr):
    ...
    # Adjust learning rate based on means
    updated_lr = lr * (torch.mean(torch.tensor([xm, ym, zm], device=prev.device)) / (xm + ym + zm))
    return updated_lr

def contrastive_loss(z_prev, z_present, z_serv, temperature=0.5, margin=0.2, lr=1e-3):
    # Normalize embeddings using PyTorch
    z_prev = F.normalize(z_prev, p=2, dim=1)       # Previous model projection
    z_present = F.normalize(z_present, p=2, dim=1) # Current model projection

    # Calculate Euclidean distance
    dis = torch.sqrt(torch.sum((z_present - z_prev) ** 2, dim=1) + 1e-7)

    # Apply margin and compute squared differences
    sqpres = torch.square(z_present) # Squared current projection
    sqMar = torch.square(torch.clamp(margin - z_present, min=0.0)) # Margin correction
    ...
    # Normalize the server representation
    srv = torch.sqrt(torch.sum(z_serv ** 2) + 1e-7)
    srv = torch.log(srv)
...
# def contrastive_loss(z_prev, z_present, z_serv, temperature=0.5, margin=0.2, lr=1e-3):
#     # **Ensure all tensors are on the same device and detach gradients**
#     z_prev = z_prev.detach().to(z_present.device)
#     z_present = z_present.to(z_present.device)
#     z_serv = z_serv.detach().to(z_present.device)

#     # **L2 Normalization using PyTorch instead of TensorFlow**
#     z_prev = torch.nn.functional.normalize(z_prev, p=2, dim=-1)
#     z_present = torch.nn.functional.normalize(z_present, p=2, dim=-1)

#     # **Compute Euclidean distance**
#     dis = torch.sqrt(torch.sum((z_present - z_prev) ** 2, dim=-1))

#     # **Ensure numerical stability (prevent zero division)**
#     dis = torch.sqrt(torch.clamp(dis, min=1e-7))

#     # **Squared margin loss**
#     sqpres = z_present ** 2
#     sqMar = (torch.clamp(margin - z_present, min=0)) ** 2
#     loss = torch.mean(z_prev * sqpres + (1 - z_prev) * sqMar)

#     # **Adjust loss based on learning rate**
#     loss = loss - (loss * lr)

#     # **Learning rate adjustment using mean reduction**
#     srv = torch.sqrt(torch.sum(z_serv ** 2, dim=-1))
#     srv = torch.log(srv)
#     return loss / srv
...
def train_net(net_id, net, train_dataloader, test_dataloader, epochs, lr, args_optimizer, args, device="cpu"):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (x, target) in enumerate(train_dataloader):
            ...
            # h - features, z - projn for contrasive, out - class preds
            h,z_present,out = net(x)
            ce_loss = criterion(out, target)

            sample_ids = [(batch_idx, i) for i in range(x.shape[0])]

            z_prev_list = []

            for sid in sample_ids:
                if sid in past_emb:
                    z_prev_list.append(past_emb[sid])
                else:
                    z_prev_list.append(torch.zeros_like(z_present[0]).cuda())

            # z_prev = torch.stack(z_prev_list)
            if len(z_prev_list) > 0:
                z_prev = torch.stack(z_prev_list)
            else:
                z_prev = torch.zeros_like(z_present).cuda()


            for sid, z in zip(sample_ids, z_present.detach()):
                past_emb[sid] = z.clone()

            if len(past_emb) > MAX_MEMORY:
                past_emb.pop(next(iter(past_emb)))

            z_serv = torch.mean(z_present, dim=0, keepdim = True).detach()

            contr_loss = contrastive_loss(z_prev,z_present,z_serv,temperature=args.temperature,margin=0.2,lr=1e-3)

            loss = ce_loss + args.mu * contr_loss
            ...
            contr_loss_collector.append(contr_loss.item())
            ce_loss_collector.append(ce_loss.item())

        epoch_loss = sum(epoch_loss_collector) / len(epoch_loss_collector)
        epoch_loss1 = sum(ce_loss_collector) / len(ce_loss_collector)
        epoch_loss2 = sum(contr_loss_collector) / len(contr_loss_collector)

        logger.info('Epoch: %d Loss: %f CE_Loss: %f Contrastive_Loss: %f' % (epoch, epoch_loss, epoch_loss1, epoch_loss2))
